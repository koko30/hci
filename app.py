...
# Page setup
st.set_page_config(page_title="Shifting Narratives prototype", page_icon="🌍", layout="centered")
...
sent_color = "MediumSeaGreen" if row_sentiment > 0.2 else ("SlateGray" if -0.2 <= row_sentiment <= 0.2 else "Crimson")
sent_label = "Positive" if row_sentiment > 0.2 else ("Neutral" if -0.2 <= row_sentiment <= 0.2 else "Negative")
...
bar_chart = alt.Chart(event_df).mark_bar(cornerRadiusTopLeft=8, cornerRadiusTopRight=8).encode(
    ...
    width=500, height=260, title="Sentiment Comparison"
)
